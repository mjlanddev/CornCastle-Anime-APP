        ...
        viewModelScope.launch {
            ...
            try {
                val query = """
                    ...
                    query {
                      trending: Page(page: 1, perPage: 15) { media(sort: TRENDING_DESC, type: ANIME, isAdult: false) { ...mediaFields } }
                      popular: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ...mediaFields } }
                      topRated: Page(page: 1, perPage: 15) { media(sort: SCORE_DESC, type: ANIME, isAdult: false) { ...mediaFields } }
                      currentlyAiring: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, status: RELEASING) { ...mediaFields } }
                      finishedAiring: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, status: FINISHED) { ...mediaFields } }
                      upcoming: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, status: NOT_YET_RELEASED) { ...mediaFields } }
                      movies: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, format: MOVIE) { ...mediaFields } }
                      action: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, genre: "Action") { ...mediaFields } }
                      romance: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, genre: "Romance") { ...mediaFields } }
                      fantasy: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, genre: "Fantasy") { ...mediaFields } }
                      sciFi: Page(page: 1, perPage: 15) { media(sort: POPULARITY_DESC, type: ANIME, isAdult: false, genre: "Sci-Fi") { ...mediaFields } }
                      mostFavorited: Page(page: 1, perPage: 15) { media(sort: FAVOURITES_DESC, type: ANIME, isAdult: false) { ...mediaFields } }
                    }
                """.trimIndent()
                ...
                _trendingAnime.value = data.trending?.media ?: emptyList()
                _popularAnime.value = data.popular?.media ?: emptyList()
                _topRatedAnime.value = data.topRated?.media ?: emptyList()
                _currentlyAiring.value = data.currentlyAiring?.media ?: emptyList()
                _finishedAiring.value = data.finishedAiring?.media ?: emptyList()
                _upcomingAnime.value = data.upcoming?.media ?: emptyList()
                _movies.value = data.movies?.media ?: emptyList()
                _actionAnime.value = data.action?.media ?: emptyList()
                _romanceAnime.value = data.romance?.media ?: emptyList()
                _fantasyAnime.value = data.fantasy?.media ?: emptyList()
                _sciFiAnime.value = data.sciFi?.media ?: emptyList()
                _mostFavorited.value = data.mostFavorited?.media ?: emptyList()
                
                _error.value = null
            } catch (e: Exception) {
                if (e is kotlinx.coroutines.CancellationException) throw e
                Log.e("MainViewModel", "Fetch home data failed: ${e.message}", e)
